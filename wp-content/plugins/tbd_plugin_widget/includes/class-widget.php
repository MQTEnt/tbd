...
if ( ! class_exists( 'WP_Widget_Black_Studio_TinyMCE' ) ) {
	...
	class WP_Widget_Black_Studio_TinyMCE extends WP_Widget {
		...
		public function __construct() {
			...
			$widget_title = __( 'Thiết bị điện: Giới thiệu công ty', 'black-studio-tinymce-widget' ); //TMQ Custom
			/* translators: description of the widget, shown in available widgets */
			$widget_description = __( 'Giới thiệu ngắn gọn về công ty', 'black-studio-tinymce-widget' );
		...
		public function widget( $args, $instance ) {
			$before_widget = '<div class="sidebar-container">'; //TMQ Custom
			$after_widget = '</div> <!-- End .sidebar-contaier -->'; //TMQ Custom
			$before_title = '<h2>'; //TMQ Custom
			$after_title = '</h2>'; //TMQ Custom
			$before_text = apply_filters( 'black_studio_tinymce_before_text', '<div class="textwidget sidebar-content about-company">', $instance ); //TMQ Custom
